...
function initialize_database()
    # print(cd(pwd))
    if isfile("total-extinctions_output.sqlite")
        error("total-extinctions_output.sqlite already exists; delete first")
    end
    db = DB("total-extinctions_output.sqlite")
     execute(db, """
        CREATE TABLE total_vextinction (
            by_time REAL,
            extinction INTEGER
        )
    """)   
...
function write_total_extinction(sim,ext)
    execute(sim.db,
        "INSERT INTO total_vextinction VALUES (?,?)",
        [sim.t, ext]
    )
end
