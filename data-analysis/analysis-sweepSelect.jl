...
ROOT_PATH = joinpath("/scratch/al8784/26_MOI3/data-analysis")
# dbSimPath = joinpath(SCRIPT_PATH, "..", "simulation", "sweep_db.sqlite")
dbSimPath = joinpath("/Volumes/Yadgah/sylvain-martin-collab/26_MOI3/sweep_db.sqlite")
...
const N_PROCESSES = 15
const N_CORES_PER_JOB_MAX = 15 # Half a node (14) is easier to get scheduled than a whole one
const mem_per_cpu = 5000 # in MB 100MB = 1 GB
...
# println("combination IDs: $(cIDs)")

cIDs = [cID for (cID,) 
            in execute(dbSim,"SELECT combo_id FROM param_combos 
                    WHERE evofunctionScale in (0,3,6,10,12)
                    AND microbe_mutation_prob_per_spacer in (0,1)
                    AND n_particles_per_vstrain > 0")]
# println("combination IDs: $(cIDs)")

# cIDs = [1 9 13 97 101 2 10 14 98 102]
println("combination IDs: $(cIDs)")
...
function main()
    ...
    numSubmits = generate_analysis_runs(dbSim,cIDs)
    generate_analysis_jobs(dbSim,dbTempJobs,numSubmits,cIDs)
end

function generate_analysis_runs(dbSim::DB,cIDs) # This function generates the directories
    ...
    for (run_id, combo_id, replicate) in execute(dbSim, "SELECT run_id,combo_id,replicate FROM runs WHERE combo_id in ($(join(cIDs, ", "))) ORDER BY combo_id, replicate")
        #println("Processing analysis script for combination $(combo_id)/replicate $(replicate)") # local
    ...
    #(run_count,) = execute(dbSim, "SELECT run_id FROM runs WHERE combo_id in ($(join(cIDs, ", "))) ORDER BY run_id DESC")
    #run_count = run_count[1]
    return numSubmits = Int64(ceil(run_count/(N_JOBS_MAX*N_PROCESSES)))
end

function generate_analysis_jobs(dbSim::DB,dbTempJobs::DB,numSubmits::Int64,cIDs)
    ...
    for (run_id, run_dir) in execute(dbSim, "SELECT run_id, run_dir FROM runs WHERE combo_id in ($(join(cIDs, ", "))) ORDER BY replicate, combo_id")
