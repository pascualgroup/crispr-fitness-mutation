...
#######
## COMPILES DATAFRAME FOR VIRAL TEMPORAL AND LOCAL ADAPTATION
#######
scale = 10
...
#######
## COMPILES DATAFRAME FOR HOST TEMPORAL ADAPTATION
#######
...
for combo in combos:
    ...
    axes[0].margins(x=0)
    axes[1].margins(x=0)
    fig.savefig(os.path.join(simDir,'adaptation-{0}-{1}_{2}-trunc.pdf'.format(divtype,int(micMutSpacer),nThresh)),dpi=resolve)
    plt.close('all')
...
#
fig.savefig(os.path.join(simDir,'supp-LA-all_{0}.pdf'.format(nThresh)),dpi=resolve)
plt.close('all')
...
axes[0].margins(x=0)
axes[1].margins(x=0)
fig.savefig(os.path.join(simDir,'supp-LA-decomposition_{0}.pdf'.format(nThresh)),dpi=resolve)
plt.close('all')
plt.show()
#####
#####
