...
#   
DBCOMBO_PATH = os.path.join(simDir,'sweep_db.sqlite')
...
fig.savefig(os.path.join(simDir,'{0}-extinction_55_quantile_mean_ALL_5offset.png'.format(treatment)),dpi=resolve)
plt.close('all')
...
for treatment in ['Monomorphic','Polymorphic']:
    ...
    fig.savefig(os.path.join(simDir,'{0}-extinction_55_quantile_mean_ALL.png'.format(treatment)),dpi=resolve)
    plt.close('all')
